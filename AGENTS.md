...
> [!NOTE]
> **Текущее состояние проекта:** 🚀 **Релизный baseline v1.0.0–v1.0.2 + Завершён сквозной UI/UX рефакторинг и QA-харденинг**  
> Содержание всех 22 тематических модулей (1413 статей) полностью написано и вычитано по Кернигановскому стандарту (исторический релизный baseline `v1.0.2`). Поверх релизной базы реализована трёхтемная дизайн-система (`paper`, `light`, `dark`), семантический HTML5, адаптивная подсветка кода Prism, тема-зависимый рендеринг Mermaid и расширенный QA-аудит (0 broken links, 0 filesystem errors, 0 Mermaid errors).
...
* **`AGENTS.md` — архитектурный регламент и операционное руководство (Agent Guide & Constraints):**  
  Свод правил, технических ограничений и процедур для инженеров и AI-агентов.
...
│   ├── colorize_mermaid.py# Утилита предварительной семантической раскраски схем Mermaid в sources/
...
5. **Семантические токены CSS Custom Properties:**  
   Все цвета компонентов задаются исключительно через переменные в блоках `[data-theme="..."]` файла `builder/assets/style.css`:
...
6. **Запрет хардкода:** В стилях компонентов категорически запрещено использовать прямые hex/rgba значения цвета там, где существует семантический токен.
...
Утилита `builder/colorize_mermaid.py` — это инструмент пакетной статической разметки схем в Markdown-исходниках (`sources/`). Её директивы `classDef` настраивают базовые цвета тёмной темы. Для тем `paper` и `light` цвета узлов переопределяются динамически в `style.css` через селекторы высокой специфичности с `!important`. **Не изменяйте `colorize_mermaid.py` без специального архитектурного поручения.**
...
3. **Комфортная ширина статьи:** Максимальная ширина текстового блока статьи ограничена переменной `--max-content-width: 860px` для поддержания оптимальной длины строки при многочасовом чтении; базовый размер шрифта 16px, межстрочный интервал `line-height: 1.7`.
...
* **Релизный baseline содержания (`v1.0.0`–`v1.0.2`):** Завершена полная авторская вычитка всех 22 модулей (1413 статей) по Кернигановскому стандарту.
...
* **Documentation Sync (commit `59533a37`):** `docs(repo): synchronize agent instructions with current architecture`  
  Актуализация документации проекта под внедрённую дизайн-систему и семантическую структуру.
