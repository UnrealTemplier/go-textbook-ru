...
> Каноническая версия проекта объявляется и поддерживается исключительно в [§ 1.4](#14-versirovanie-proekta-canonical-version).  
...
|---|:---:|---|
| 🌑 **Dark** | `dark` | **Тема по умолчанию.** Инженерная рабочая станция Unix / ретро-терминал. Глубокая тёмная палитра почти чёрного древесного угля (`#11100e` / `#181613` / `#201d19`), тёплые янтарные (`#d89b32`) и терракотовые (`#e06b2d`) акценты, полное отсутствие сине-фиолетовой неоновой дымки. Минимальная зрительная нагрузка при длительной работе с кодом. |
| 📄 **Paper** | `paper` | Эстетика классической инженерной книги, университетского справочника Bell Labs. Тёплая состаренная бумага и пергамент (`#f3ecda` / `#eae0c8` / `#fbf7ec`), глубокий тёплый текст (`#26211a`), акценты жжёного апельсина / янтаря (`#b45309`) и ржавчины (`#8c3b12`), янтарная терминальная подсветка кода (`#181512` / `#edd8b4`). Нулевая зрительная усталость при чтении. |
| ☀️ **Light** | `light` | Современная чистая инженерная документация. Нейтральный светлый технический фон (`#f7f8fa` / `#eef0f3` / `#ffffff`), уверенный тёмный текст (`#101828`), строгий инженерный синий акцент (`#1e5a96` / `#18528c`) без фиолетовой дымки, чёткие структурные разделители (`#d0d5dd` / `#98a2b3`). |
...
5. **Семантические токены CSS Custom Properties:**  
   ...
   *Канонические базовые значения ключевых токенов палитр:*
   ...
   | Семантический токен | Dark (по умолчанию) | Paper | Light |
   |---|:---:|:---:|:---:|
   | `--bg` (основной холст страницы) | `#11100e` | `#f3ecda` | `#f7f8fa` |
   | `--bg-surface` (сайдбар и панели) | `#181613` | `#eae0c8` | `#eef0f3` |
   | `--bg-card` (карточки и выноски) | `#201d19` | `#fbf7ec` | `#ffffff` |
   | `--text` (основной текст) | `#ded8cc` | `#26211a` | `#101828` |
   | `--text-secondary` (второстепенный текст) | `#b8b0a2` | `#3b342a` | `#242c38` |
   | `--text-muted` (приглушенный текст) | `#8a8275` | `#695e4f` | `#475467` |
   | `--border` (основные границы) | `#302a23` | `#d4c7af` | `#d0d5dd` |
   | `--border-strong` (акцентные границы) | `#4d4338` | `#b2a184` | `#98a2b3` |
   | `--accent` (основной акцент) | `#d89b32` (тёплый янтарь) | `#b45309` (жжёный апельсин) | `#1e5a96` (инженерный синий) |
...
* **Лэндмарки документа:**
  ...
  - `<main class="content-wrapper" id="main-content">` — главная контентная область статьи.
  - `<header class="index-hero">` — верхняя героическая секция главной страницы, содержит метаданные (`.hero-top-meta`) с бейджем курса и версией (`.hero-version`).
...
* **Layout & Palette Refinement (коммит `9ec687c3`):** `style(ui): refine theme palettes and content width`  
  Расширение доступной ширины статьи до 1040px, ограничение колонки прозы до 880px, устранение сжатия Mermaid и таблиц, колористический полиш палитр трёх тем.
* **Offline First Label Removal (коммит `8e180057`):** `chore(ui): remove offline first sidebar label`  
  Удаление визуального бейджа Offline First из футера сайдбара (сохранение offline-first на уровне архитектурного принципа проекта).
* **Unified Floating Theme Switcher (коммит `143867aa`):** `feat(ui): add unified floating theme switcher`  
  Вынос переключателя тем из сайдбара в единую плавающую кнопку в правом нижнем углу экрана (icon-only, SVG-иконки paper/sun/moon, доступный `aria-label`).
* **Dark Theme Default (коммит `254f03d1`):** `style(ui): make dark theme default`  
  Назначение темы Dark темой по умолчанию для новых пользователей с сохранением приоритета сохранённого пользовательского выбора в `localStorage`.
