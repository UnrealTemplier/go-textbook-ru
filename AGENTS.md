...
7. **Чистая структура репозитория и разграничение зон ответственности:**
   ...
   - `favicon.ico`, `favicon.svg`, `.gitignore` — Вспомогательные системные файлы. Корень репозитория содержится в абсолютной чистоте (никаких отладочных скриншотов, временных логов, планов или черновиков).
...
9. ❌ **НЕ засорять корень репозитория:**
   В корне проекта разрешены только: `sources/`, `builder/`, `dist/`, `AGENTS.md`, `favicon.ico`, `favicon.svg`, `.gitignore`. Запрещено складывать в корень скриншоты (`.png`), временные планы (`*_PLAN.md`), дампы json или текстовые файлы заметок.
