...
> [!NOTE]
> **Текущее состояние проекта:** 🚀 **Релиз v1.0.0 (Стабильный)**  
> Все 22 модуля (1413 статей, 1414 HTML-страниц) полностью написаны, отредактированы по Кернигановскому стандарту и скомпилированы. Пройден сквозной QA-аудит (0 broken links, 0 filesystem errors). Энциклопедия готова к чтению и автономной работе.
...
7. **Чистая структура репозитория и разграничение зон ответственности:**
   - `sources/` — **Единый источник правды (Single Source of Truth) содержания.** Все 22 модуля (1413 `.md` файлов) хранятся здесь. Любые правки контента вносятся исключительно в `sources/`.
   - `builder/` — **Движок генератора (SSG на Python).** Парсеры, шаблонизаторы, стили, JS, шрифты KaTeX и QA-скрипты.
   - `dist/` — **Автономный скомпилированный сайт (артефакт сборки).** Содержит 1414 HTML-страниц и статические ассеты. Коммитится в Git для возможности моментального чтения через `file:///` и развертывания на любом статическом хостинге. Ручное редактирование файлов в `dist/` категорически запрещено (затирается при сборке).
   - `AGENTS.md` — **Центр управления проектом, архитектурный манифест и единый регламент для AI-агентов.**
   - `README.md`, `favicon.ico`, `favicon.svg`, `.gitignore` — Вспомогательные системные файлы. Корень репозитория содержится в абсолютной чистоте (никаких отладочных скриншотов, временных логов, планов или черновиков).
...
## 📊 5. Сводные метрики базы знаний (Релиз v1.0.0)

| Метрика | Значение | Примечание |
|---|:---:|---|
| **Всего тематических модулей** | **22** | Охват от кремния до System Design и собеседований (100% завершены) |
| **Всего инженерных статей (.md)** | **1413** | 0 пустых файлов, 100% статей наполнены контентом |
| **Всего сгенерированных страниц HTML** | **1414** | 1413 статей + главная `dist/index.html` со сквозным дашбордом |
| **Всего диаграмм Mermaid** | **2486** | Все диаграммы оформлены в тёмной семантической палитре (§2.5.2) |
| **Внутренних ссылок (Wikilinks)** | **7068** | Полносвязный граф понятий с канонизацией суррогатов |
| **Блоков-выносок (Callouts)** | **4103** | `[!tip]`, `[!warning]`, `[!info]`, `[!note]`, `[!interview]`, `[!caution]`, `[!important]` |
| **Общий объем markdown-текста** | **34.50 МБ** | Эквивалент 15-томного фундаментального академического издания |
...
```
builder/
├── scanner.py          # Сканирование ./sources, нормализация метаданных, построение графа wikilinks, поддержка спецсимволов (# в C#) и защита от заголовков в code-блоках
├── converter.py        # Markdown AST парсер: callouts, Mermaid, Prism.js, защита заголовков в fenced code-блоках для TOC, адаптивные таблицы
├── template.py         # HTML5 каркас, CSS Grid/Flexbox стили, JS (живой поиск, resize сайдбара, KaTeX, Prism, Mermaid, zoom)
├── build_all.py        # Главный пайплайн сборки: флаги --all, --module N, --pilot. Сборка всего портала (~1400 страниц) за ~13 сек
├── audit_all.py        # Сквозной QA аудит: кроссплатформенность имен (Windows/macOS/Linux), 0 broken links, валидация анкоров и Mermaid
├── colorize_mermaid.py # Утилита семантической раскраски диаграмм Mermaid по канонической палитре §2.5.2
...
└── assets/             # Локальные стили, скрипты и автономные web-шрифты KaTeX для Offline First
```
...
- [x] **Шаг 5: Итеративная глубокая авторская редактура (Кернигановский стандарт)**
  ...
  - [x] Модуль 21 («21. Задачи с LeetCode и собеседований на Go», 247 задач) — 100% верифицировано и переписано в Кернигановском стандарте (247/247 задач, 306 схем Mermaid, все 12 этапов закрыты).
  - [x] Модуль 22 («22. Вопросы и ответы с собеседований по Go (Теория)», 28 лекций) — 100% верифицировано и отрефакторено в Кернигановском стандарте (28/28, 70 Mermaid-диаграмм, 31 новый вопрос по Go 1.22–1.27, Q01–Q31).
- [x] **Шаг 6: Финализация, закрытие технического долга и релиз v1.0.0**
  - [x] Решение проблемы wikilinks со спецсимволами: устранена ошибка ложного якоря `#item` при ссылках на статьи с `#` в названии (например, `C#`, `F#`).
  - [x] Защита оглавления (TOC): игнорирование комментариев Markdown (`## `) внутри fenced code-блоков (`Makefile`, `Go`) в модулях `scanner.py` и `converter.py`.
  - [x] Семантическая раскраска 100% Mermaid-диаграмм: приведение к палитре §2.5.2 (контрастный белый текст `#f8fafc` на тёмном фоне).
  - [x] Полный перевод всех цитат и эпиграфов (268 цитат) классиков Computer Science на русский язык в Кернигановском стиле (§2.4).
  - [x] Сквозной QA-аудит качества (`builder/audit_all.py`): 0 ошибок кроссплатформенности имен файлов, 0 битых ссылок (broken links), 0 ошибок синтаксиса Mermaid.
  - [x] Генеральная уборка репозитория: удаление отладочных скриншотов, устаревших планов и артефактов инициализации.
  - [x] Выпуск официального релизного тега `v1.0.0` (сентябрь 2026).
- [ ] **Шаг 7: Долгосрочная поддержка и эволюция энциклопедии**
  - [ ] Актуализация под новые релизы Go (1.25, 1.26, 1.27) по мере стабилизации фичей рантайма и стандартной библиотеки.
  - [ ] Расширение задачников новыми паттернами HighLoad и сценариями с реальных собеседований.
...
- **Статус:** ✅ Завершен (100% переписан в Кернигановском стиле: 247/247 статей, все 12 этапов выполнены)
- **Статей:** `247` | **Диаграмм Mermaid:** `306` | **Объем:** `3033.8 КБ` | **Подразделов:** `31`
...
| 02. Задачи / 08. Связные списки | 9 | 9 | 99.9 | ✅ Завершен (9/9) |
| 02. Задачи / 09. Бинарный поиск | 9 | 9 | 90.7 | ✅ Завершен (9/9) |
| 02. Задачи / 10. Backtracking | 10 | 10 | 162.6 | ✅ Завершен (10/10) |
| 02. Задачи / 11. Поиск в глубину DFS | 8 | 8 | 132.4 | ✅ Завершен (8/8) |
| 02. Задачи / 12. Поиск в ширину BFS | 8 | 8 | 88.8 | ✅ Завершен (8/8) |
| 02. Задачи / 13. Деревья | 8 | 8 | 93.3 | ✅ Завершен (8/8) |
| 02. Задачи / 14. Графы | 9 | 11 | 111.3 | ✅ Завершен (9/9) |
| 02. Задачи / 15. Union Find | 7 | 7 | 76.7 | ✅ Завершен (7/7) |
| 02. Задачи / 16. Кучи и Top K | 7 | 6 | 75.0 | ✅ Завершен (7/7) |
| 02. Задачи / 17. Жадные алгоритмы | 8 | 9 | 122.2 | ✅ Завершен (8/8) |
| 02. Задачи / 18. Динамическое программирование 1D | 7 | 7 | 72.9 | ✅ Завершен (7/7) |
| 02. Задачи / 19. Динамическое программирование 2D | 7 | 7 | 82.7 | ✅ Завершен (7/7) |
| 02. Задачи / 20. Динамическое программирование на строках | 6 | 6 | 68.3 | ✅ Завершен (6/6) |
| 02. Задачи / 21. Динамическое программирование на деревьях | 5 | 4 | 57.0 | ✅ Завершен (5/5) |
| 02. Задачи / 22. Битовые операции | 7 | 4 | 137.6 | ✅ Завершен (7/7) |
| 02. Задачи / 23. Интервалы | 6 | 6 | 103.2 | ✅ Завершен (6/6) |
| 02. Задачи / 24. Матрицы и grid задачи | 7 | 7 | 117.7 | ✅ Завершен (7/7) |
| 02. Задачи / 25. Строки | 7 | 19 | 136.1 | ✅ Завершен (7/7) |
| 02. Задачи / 26. Design задачи и структуры данных | 7 | 13 | 117.2 | ✅ Завершен (7/7) |
| 02. Задачи / 27. Рандомизированные алгоритмы | 4 | 4 | 44.8 | ✅ Завершен (4/4) |
| 02. Задачи / 28. Потоковые алгоритмы | 5 | 6 | 57.5 | ✅ Завершен (5/5) |
| 02. Задачи / 29. Сложные задачи | 7 | 8 | 73.9 | ✅ Завершен (7/7) |

### Раздел 22. 22. Вопросы и ответы с собеседований по Go (Теория)
- **Статус:** ✅ Завершен (100%) (28/28 лекций вычитано, обогащено и верифицировано в Кернигановском стандарте)
- **Статей:** `28` | **Диаграмм Mermaid:** `70` | **Объем:** `6621.0 КБ` | **Подразделов:** `1`

> Единый фундаментальный модуль из 28 исчерпывающих тем: от базовых структур языка до архитектуры рантайма, сборщика мусора, сетей, Linux, System Design на 1M RPS и вопросов уровня Staff/Principal. Включает 31 новый вопрос по Go 1.22–1.27, 70 цветных Mermaid-схем, интерактивные блоки `[!INTERVIEW]` и канонические эпиграфы классиков Computer Science.

---

## 🧭 9. Руководство для будущих агентов: Как дорабатывать проект и чего делать категорически нельзя

Этот раздел является практическим руководством для любой модели ИИ, подключающейся к проекту без знания предшествующей истории переписки.

### 9.1. Архитектурная картина мира (Кратко)
1. **Проект завершен и стабилен:** Все 22 модуля (1413 статей) полностью написаны, вычитаны и скомпилированы в `dist/`. Выпущен стабильный релиз `v1.0.0`.
2. **Offline First:** Сайт открывается прямо из файловой системы через `file:///.../dist/index.html` без веб-сервера. Внешние CDN запрещены.
3. **Разделение зон:** Исходники живут в `sources/`, генератор — в `builder/`, результат — в `dist/`. Изменения вносятся **только в `sources/`**, после чего запускается сборщик.

### 9.2. Инженерный рабочий цикл (Workflow внесения изменений)
Любое изменение в энциклопедии обязано следовать строгому конвейеру:

1. **Авторизация задачи (§0):**
   - Работать **только** по прямому запросу пользователя.
   - Строго соблюдать заданные границы (scope): если просят исправить опечатку в лекции 3 модуля 5 — менять **только** эту лекцию. Не трогать другие файлы "заодно".

2. **Внесение правок в исходники:**
   - Редактировать соответствующие файлы исключительно внутри каталога `sources/`.
   - Сохранять Кернигановский стандарт (§2), нулевую потерю знаний (§3), русский язык эпиграфов (§2.4) и тёмную семантическую палитру Mermaid (§2.5).

3. **Сборка проекта:**
   - Для быстрой проверки конкретного модуля:  
     `python3 builder/build_all.py --module <N>` (~1-2 сек)
   - Для полной пересборки портала:  
     `python3 builder/build_all.py --all` (полная компиляция 1413 статей за ~13 сек)

4. **Обязательный сквозной QA-аудит:**
   - Запустить скрипт аудита:  
     `python3 builder/audit_all.py`
   - **Критерий успеха:** скрипт обязан завершиться вердиктом:  
     `🎉 АУДИТ ПРОЙДЕН УСПЕШНО!`  
     - Ошибок несовместимости имен файлов: `0`
     - Битых ссылок (Broken Links): `0`
     - Ошибок диаграмм Mermaid: `0`

5. **Фиксация в Git:**
   - Добавить изменённые файлы из `sources/` и пересобранные из `dist/`:  
     `git add sources/ dist/`
   - Создать атомарный коммит в Conventional Commits стиле (`fix:`, `feat:`, `docs:`, `chore:`).

---

### 9.3. Категорические запреты (Strict Anti-Patterns — «Чего делать НЕЛЬЗЯ»)

1. ❌ **НЕ начинать работу самовольно без команды пользователя (§0):**
   Категорически запрещено "улучшать" соседние статьи, начинать рефакторинг других модулей или проводить "инициативные зачистки". Делать ровно то, о чем явно попросил пользователь.
2. ❌ **НЕ редактировать сгенерированный HTML в каталоге `dist/` напрямую:**
   Каталог `dist/` — производный артефакт. При следующем запуске `build_all.py` все ручные правки в HTML будут безвозвратно перезаписаны.
3. ❌ **НЕ нарушать кроссплатформенность файловой системы (§1.4):**
   Строжайше запрещены символы `:`, `*`, `?`, `"`, `<`, `>`, `|`, `\` в именах файлов и папок (ломает `git checkout` на Windows). Запрещены завершающие точки и пробелы в именах, а также зарезервированные имена DOS (`CON`, `NUL` и т.д.).
4. ❌ **НЕ использовать светлые цвета и фоны в диаграммах Mermaid (§2.5):**
   Запрещены светлые fill-цвета (`#ffffff`, `#ff6b6b`, `#c8e6c9` и др.) без тёмного текста. Все блоки диаграмм обязаны использовать тёмные тона из семантической палитры §2.5.2 и содержать `color:#f8fafc` (контрастный белый текст).
5. ❌ **НЕ оставлять англоязычные цитаты в эпиграфах (§2.4):**
   Все цитаты классиков (Кернигана, Ритчи, Томпсона, Кнута и др.) обязаны быть переведены на литературный русский язык в формате:
   ```markdown
   > *«Текст цитаты на русском языке».*  
   > — Автор
   ```
6. ❌ **НЕ использовать устаревшие суррогаты Obsidian в тексте и заголовках (§1.5):**
   Строго канонические термины: `CI/CD`, `TCP/IP`, `I/O`, `net/http`, `sync/atomic`. Суррогаты с подчеркиваниями (`CI_CD`, `TCP_IP`, `net_http`) запрещены.
7. ❌ **НЕ помещать Markdown-заголовки `## ` внутрь code-блоков:**
   Хотя парсер генератора умеет фильтровать fenced-блоки, не следует оформлять комментарии внутри кода как Markdown-заголовки. Используйте стандартные `# target:` (в Makefile) или `// note:` (в Go).
8. ❌ **НЕ урезать техническую фактуру при правках (Zero Knowledge Loss §3):**
   Категорически запрещено удалять имена системных вызовов, регистров, алгоритмические детали, вопросы собеседований или предупреждения об ошибках.
9. ❌ **НЕ засорять корень репозитория:**
   В корне проекта разрешены только: `sources/`, `builder/`, `dist/`, `AGENTS.md`, `README.md`, `favicon.ico`, `favicon.svg`, `.gitignore`. Запрещено складывать в корень скриншоты (`.png`), временные планы (`*_PLAN.md`), дампы json или текстовые файлы заметок.
10. ❌ **НЕ подключать внешние сетевые зависимости (Offline First §1.1):**
    Запрещено вставлять ссылки на внешние CDN для стилей, шрифтов или скриптов (`fonts.googleapis.com`, `cdnjs`, `unpkg` и др.). Все ресурсы обязаны поставляться локально в `dist/assets/`.
