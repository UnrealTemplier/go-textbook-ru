...
В золотую эпоху лабораторий Bell Labs был сформулирован бессмертный Unix-принцип: создавайте маленькие, острые программы, каждая из которых решает ровно одну задачу, но решает её безупречно. В мире компьютерных сетей инструментами первостепенной важности стали утилиты `ping`, `traceroute`, `dig`, `curl` и `mtr`.
...
Словно гидролокатор подводной лодки, посылающий акустический импульс сквозь толщу океана, `ping` отправляет короткий пакет и засекает время до прихода отраженного эха.
...
flowchart TD
    ...
    subgraph SocketTypes ["Иерархия типов сокетов в Linux"]
        Stream["SOCK_STREAM (TCP): Ядро управляет окном, SYN/ACK, ретрансмиссией"]
        Dgram["SOCK_DGRAM (UDP): Приложение задает payload, ядро ставит UDP-хедер"]
        Raw["SOCK_RAW (ICMP / Raw IP): Приложение само формирует заголовки L3/L4"]
    end
```
...
Представьте себе почтовую открытку, на которой написано: «Уничтожить после одной пересылки». Первое же почтовое отделение, принявшее открытку, гасит её и отправляет вам служебное уведомление: «Ваша открытка погибла на станции №1». Вы отправляете следующую открытку с лимитом в две пересылки — и получаете уведомление со станции №2. Шаг за шагом вы картографируете весь магистральный маршрут.
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    Client["Клиент\nTTL=1"]:::entry -->|Отправка| R1["Маршрутизатор 1\nTTL -> 0"]
...
Инструменты диагностики наглядно демонстрируют взаимодействие пользовательского пространства с ядром Linux:
