В хорошей программе код обработки ошибок занимает больше места, чем так называемый «счастливый путь» (happy path). В реальных распределенных системах всё, что может пойти не так, обязательно пойдет не так: удаленный сервер закроет соединение, диск переполнится, база данных исчерпает пул подключений, а клиент пришлет поврежденный JSON.

Во многих языках программирования ошибки прячут за механизмом исключений (`try-catch-finally`). Исключения создают иллюзию чистоты кода, но на деле работают как невидимый оператор `GOTO`, разрушая предсказуемость исполнения программы и заставляя процессор дорого платить за раскрутку стека. 

Создатели Go Роб Пайк и Кен Томпсон вернулись к истокам Unix и языка C: **ошибки — это обычные значения (Errors are values)**. Они не прерывают ход времени, не ломают конвейер процессора и обрабатываются явно там, где они возникли.
...
flowchart TD
    ...
    classDef ok fill:#064e3b,stroke:#10b981,stroke-width:2px,color:#f8fafc;
...
В production-сервисах ошибки делятся на категории: доменные, инфраструктурные и системные. Сервис не должен возвращать инфраструктурные ошибки СУБД или сетевые сбои напрямую. Он маппит их в доменные ошибки, сохраняя контекст.
...
func (s *UserService) Create(ctx context.Context, req *CreateUserReq) error {
    ...
    if err := s.repo.Create(ctx, &User{Email: req.Email}); err != nil {
        // Ошибку нарушения уникальности из репозитория превращаем в доменный бизнес-сигнал
