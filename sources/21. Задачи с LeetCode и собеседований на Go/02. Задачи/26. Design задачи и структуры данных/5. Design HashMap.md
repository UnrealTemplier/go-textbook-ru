...
| **Swiss Tables (Go 1.24+)** | Группы по 16 слотов с контрольными байтами (Ctrl bytes) и SIMD/SWAR поиском. | Параллельный опрос 16 слотов за 1-2 инструкции CPU; минимальные накладные расходы памяти. | Сложность низкоуровневой логики зондирования. | Go 1.24+, Abseil C++, Rust `std::collections::HashMap`. |
...
4. В Go 1.24+ архитектура ассоциативных массивов перешла на Swiss Tables, сочетающие 16-слотовые группы с SIMD/SWAR фильтрацией по контрольным байтам.
